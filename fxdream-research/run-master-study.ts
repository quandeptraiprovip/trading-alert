/**
 * fxdream-research/run-master-study.ts
 *
 * Script kiểm chứng Master Engine (Key Volume Kép + FVG + EQH Sweep + Daily Trap Gate).
...
import {
  MASTER_FXDREAM_PARAMS,
  FXDreamMasterParams,
  findKeyVolumeLevelsMaster,
  evaluateDailyContextMaster,
  detectSFPSignalsMaster,
  buildSignalPlanMaster,
  SignalPlanMaster,
  TradeResultMaster,
} from "./strategy-engine-master";
...
function runMasterBacktest(
  ...
  const trades: TradeResultMaster[] = [];
  if (c5m.length < 500 || c15m.length < 200 || c1h.length < 100) {
    return createEmptySummary(symbol);
  }

  const allKeys = findKeyVolumeLevelsMaster(c1h, c4h, params);

  let pendingPlan: SignalPlanMaster | null = null;
  let activePosition: {
    plan: SignalPlanMaster;
    entryTime: number;
    entryPrice: number;
    currentSL: number;
    targetPrice: number;
  ...
  for (let i = start5mIdx; i < c5m.length; i++) {
    ...
    if (nowTime % TF_MS["15m"] === 0) {
      ...
      const dailyCtx = evaluateDailyContextMaster(validDaily, nowTime, params);
      const sfpEvents = detectSFPSignalsMaster(valid15m, allKeys, dailyCtx, params);
      ...
      if (sfpEvents.length > 0) {
        ...
        const plan = buildSignalPlanMaster(symbol, latestEvent, opposingTarget, params);
...
async function main() {
  ...
  console.log(`================================================================`);
  console.log(` KIỂM CHỨNG BỘ THUẬT TOÁN MASTER ENGINE (KEY KÉP + FVG + EQH)`);
  console.log(`================================================================\n`);
  ...
  console.log(`\n[2/2] Đang chạy mô phỏng Master Engine...`);

  // Master Engine: Standard Binance Futures Fee
  const summaryMaster = runMasterBacktest(symbol, c5m, c15m, c1h, c4h, c1d, MASTER_FXDREAM_PARAMS);

  // Master Engine: Low-Fee Venue (Vàng / Forex / Zero Maker Fee)
  const lowFeeParams: FXDreamMasterParams = {
    ...MASTER_FXDREAM_PARAMS,
    makerFeeRate: 0.0001,
    takerFeeRate: 0.0001,
    slippageRate: 0.0,
  };
  const summaryMasterLowFee = runMasterBacktest(symbol, c5m, c15m, c1h, c4h, c1d, lowFeeParams);

  console.log(`\n========================================================================================================`);
  console.log(`           BẢNG KẾT QUẢ MÔ PHỎNG CHIẾN LƯỢC MASTER FX DREAM TRADING (365 NGÀY)`);
  console.log(`========================================================================================================`);
  console.log(
  ...
  printRow("Master Engine (Binance Futures Phí Standard)", summaryMaster);
  printRow("Master Engine (Venue Phí Thấp / Vàng-Forex)", summaryMasterLowFee);

  console.log(`========================================================================================================\n`);
}

main().catch((err) => {
  console.error("Lỗi chạy Master Study:", err);
  process.exit(1);
});
